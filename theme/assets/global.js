...
class MegaMenuTabs extends HTMLElement {
  ...
  connectedCallback() {
    ...
    this.titles = this.querySelectorAll('.mega-menu__list-title .mega_menu-heading[data-handle]');
    this.items  = this.querySelectorAll('.megamneu-item'); // all switchable groups (UL + banner)

    if (!this.titles.length || !this.items.length) return;
    ...
    this.titles.forEach((t, idx) => {
      ...
      t.dataset.index = String(idx + 1);
      t.addEventListener('click', this.onTitleClick);
      t.addEventListener('keydown', this.onTitleKey);
    });

    // Init with first available title
    const first = this.titles[0];
    const initialHandle = first?.dataset.handle || 'megamneu-1';
    this.show(initialHandle);
  }

  disconnectedCallback() {
    this.titles?.forEach((t) => {
      t.removeEventListener('click', this.onTitleClick);
      t.removeEventListener('keydown', this.onTitleKey);
    });
  }

  onTitleClick(e) {
    const handle = e.currentTarget?.dataset?.handle;
    if (handle) this.show(handle);
  }
  ...
  show(handle) {
    ...
    // Activate clicked title
    const activeTitle = Array.from(this.titles).find(t => t.dataset.handle === handle);
    if (activeTitle) {
    ...
    // Show all items that carry the handle class (e.g., .megamneu-2)
    ...
    // Optional event for hooks/analytics
    this.dispatchEvent(new CustomEvent('megamenu:tabchange', {
      bubbles: true,
      detail: { handle }
    }));
  }
...
customElements.define('header-menu-item', MegaMenuTabs);
